...
| 三级文字 | `rgba(245,245,245,0.36)` | 数据来源标注。**不要降到 0.35 以下** —— 0.32 在 `#0F1115` 上只有 2.73:1，`hyperframes check` 的 Contrast 会判不达标（需 3:1）；0.36 为 3.15:1 |
...
**① count-up 的起点必须对齐元素出现的那一刻。**
数字元素先淡入、count-up 晚 0.3s 才开始，观众会看到一个静态的 `0.0%` 摆在那里，读起来像卡住而不像动效。同一个 tween 位置：元素 `opacity 0→1` 与 count-up 同时起。

例外：count-up 的起始值本身有意义时（如「2021 ¥28.6 → 16.2」从 28.6 数下来），静态起始值可以先摆，因为它是信息不是占位。

**② 柱顶数字标签要显式设 `line-height: 1`。**
默认 `line-height: 1.2` 会让文字盒比字号高 20%，盒底压进柱体、把数字下半截切掉。设成 1 之后盒高等于字号，柱顶留白才算得准：

```css
.bar-label { line-height: 1; }        /* 盒高 == font-size */
/* 定位：top = 柱顶 y − 字号 − 12（12 = 柱顶留白） */
```

**③ 每个图表场景都要有 Y 轴刻度标签。**
只画网格线不标数值，网格线就是纯装饰。同一支片子里若某个场景省了刻度，交叉转场时坐标轴会「突然消失」，破坏「保留坐标轴不动」的连续感。

**④ 末端数值标注要右对齐到安全边距内。**
折线末端在 `x = 图表右边界` 时，标注若用 `left` 定位会伸出安全区。改成固定宽度的右对齐盒，盒右边缘 = `画宽 − pad-x`。
